...
```{r ans51}
# twDates2
twDates2 <- str_c(str_pad(yy,3,"left","0"),"年",mm)
twDates2
```
...
```{r ans53}
# westDates
map(dateList,~.[[2]]) -> mm  #為什麼要再取一次mm而不原本的?
...
failRate
#我錯的
c(sampledTranscriptData$科目名稱==科目204)
```
...
系級<- sampledTranscriptData$系級
sampledTranscriptData$學系 <- str_replace_all(sampledTranscriptData,"c(1,2,3,4,5,6)","" ) 
